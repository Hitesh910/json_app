...
import 'package:json_app2/screen/home/model/photos_model.dart';

import '../screen/home/model/comment_model.dart';
...
class JsonHelper {
  ...
  Future<List<PostModel>> postJson() async {
    ...
    List<PostModel> postList = json.map((e) => PostModel.mapToModel(e))
        .toList();
    return postList;
  }
  ...
  Future<List<UsersModel>> usersJson() async {
    ...
    List json = jsonDecode(jsonString);
    List<UsersModel> userList = json.map((e) => UsersModel.mapToModel(e))
        .toList();
    return userList;
  }

  Future<List<CommentsModel>> commentsJson() async {
    var jsonString = await rootBundle.loadString("assets/json/comments.json");
    List json = jsonDecode(jsonString);
    List<CommentsModel> commentsList = json.map((e) =>
        CommentsModel.mapToModel(e)).toList();
    return commentsList;
  }
  ...
  Future<List<PhotosModel>> photosJson() async {
    ...
    List json = jsonDecode(jsonString);
    List<PhotosModel> photosList = json.map((e) => PhotosModel.mapToModel(e)).toList();
    return photosList;
  }
